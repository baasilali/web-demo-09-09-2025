...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[7]
    ...
    // Map neural network output to pink and black color scheme
    vec3 neuralOutput = buf[0].xyz;
    
    // Define our color palette
    vec3 black = vec3(0.0, 0.0, 0.0);           // Pure black
    vec3 darkMagenta = vec3(0.4, 0.1, 0.4);     // Dark magenta
    vec3 brightMagenta = vec3(1.0, 0.2, 0.8);   // Bright magenta
    vec3 lightMagenta = vec3(1.0, 0.5, 0.9);    // Light magenta
    
    // Use neural output to blend between colors
    vec3 finalColor = mix(black, darkMagenta, neuralOutput.x);
    finalColor = mix(finalColor, brightMagenta, neuralOutput.y * 0.7);
    finalColor = mix(finalColor, lightMagenta, neuralOutput.z * 0.4);
    
    // Create intensity mask - some areas stay black, others become pink
    float intensity = (neuralOutput.x + neuralOutput.y + neuralOutput.z) / 3.0;
    float threshold = smoothstep(0.3, 0.7, intensity);
    
    // Final blend between black and our pink variations
    finalColor = mix(black, finalColor, threshold);
  ...
  useGSAP(
    () => {
      ...
      document.fonts.ready.then(() => {
        ...
        if (badgeRef.current && badgeText && badgeLabel) {
        ...
        const microItems = microDetails && microDetails.length > 0 ? [microItem1Ref.current, microItem2Ref.current, microItem3Ref.current].filter(Boolean) : [];
